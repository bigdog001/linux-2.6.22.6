						     ...
						     void *data, int silent)
{
	...
	printk(KERN_INFO "yaffs_version = %d \n", yaffs_version);
	printk(KERN_INFO "yaffs_read_super: block size %d \n", (int)(sb->s_blocksize));
	printk(KERN_INFO "yaffs: Attempting MTD mount of %u.%u,\"%s\" \n", 
	...
	mtd = get_mtd_device(NULL, MINOR(sb->s_dev));
	if (IS_ERR(mtd)) {
		printk(KERN_INFO, "yaffs: MTD device %u either not valid or unavailable", MINOR(sb->s_dev));
	...
	if (yaffs_auto_select && yaffs_version == 1 && WRITE_SIZE(mtd) >= 2048) {
		printk(KERN_INFO, "auto selecting yaffs2");
	...
	if (yaffs_version == 2 && !options.inband_tags
	    && WRITE_SIZE(mtd) == 512) {
	    printk(KERN_INFO, "auto selecting yaffs1");
...
{
	/* Install the proc_fs entries */
	//创建 yaffs文件, YPROC_ROOT 表示上层目录为 /proc
